...
std::string_view kind_name(Token::Kind k) noexcept {
  ...
  case K::Add:      return "+";
  case K::Sub:      return "-";
  case K::Multiply: return "*";
  case K::Divide:   return "/";
  case K::Modulo:   return "%";
  case K::Pow:      return "**";
  // Bitwise
  case K::BitwiseAnd: return "&";
  case K::BitwiseOr:  return "|";
  case K::BitwiseXor: return "^";
  case K::BitwiseNot: return "~";
  case K::LeftShift:  return "<<";
  case K::RightShift: return ">>";
  // Comparison / logical
  case K::Equal:            return "==";
  case K::NotEqual:         return "!=";
  case K::LessThan:         return "<";
  case K::LessThanEqual:    return "<=";
  case K::GreaterThan:      return ">";
  case K::GreaterThanEqual: return ">=";
  case K::LogicalAnd:       return "&&";
  case K::LogicalOr:        return "||";
  // Unary
  case K::Not: return "!";
  // Assignment
  case K::Assignment:    return "=";
  case K::DeclAssignment: return ":=";
  case K::AddAssignment: return "+=";
  case K::SubAssignment: return "-=";
  case K::MulAssignment: return "*=";
  case K::DivAssignment: return "/=";
  default:               return "?";
  }
}
...
void dump_ptr(const NodePtr& p, std::ostream& os, int indent) {
  if (p) dump_impl(*p, os, indent);
  else   os << pad(indent) << "(null)\n";
}

void dump_opt_ptr(const std::optional<NodePtr>& opt,
                  std::ostream& os, int indent) {
  if (opt && *opt) dump_impl(**opt, os, indent);
}
...
void dump_identifier(const IdentifierNode& n,
                     std::ostream& os, int indent) {
  os << pad(indent) << "IdentifierNode \"" << n.name << "\"\n";
}

void dump_identifier_list(const IdentifierListNode& n,
                          std::ostream& os, int indent) {
...
void dump_field_spec(const FieldSpecNode& n,
                     std::ostream& os, int indent) {
...
void dump_field_assignment(const FieldAssignmentNode& n,
                           std::ostream& os, int indent) {
  os << pad(indent) << "FieldAssignmentNode \"" << n.name.name << "\"\n";
  dump_ptr(n.value, os, indent + 1);
}

void dump_key_value(const KeyValueNode& n,
                    std::ostream& os, int indent) {
...
void dump_opt_generic(const std::optional<GenericNode>& opt,
                      std::ostream& os, int indent) {
  if (opt) dump_generic(*opt, os, indent);
}

void dump_parameter(const ParameterNode& n,
                    std::ostream& os, int indent) {
...
void dump_signature(const SignatureNode& n,
                    std::ostream& os, int indent) {
...
void dump_case_arm(const CaseArmNode& n,
                   std::ostream& os, int indent) {
...
void dump_enum_field(const EnumFieldNode& n,
                     std::ostream& os, int indent) {
...
void dump_receiver(const ReceiverNode& n,
                   std::ostream& os, int indent) {
  os << pad(indent) << "ReceiverNode \"" << n.name.name << "\"\n";
  dump_ptr(n.type, os, indent + 1);
}

void dump_interface_field(const InterfaceFieldNode& n,
                          std::ostream& os, int indent) {
  os << pad(indent) << "InterfaceFieldNode"
     << (n.is_public ? " pub" : "")
     << " \"" << n.name.name << "\"\n";
  dump_signature(n.signature, os, indent + 1);
}

void dump_struct_member(const StructMemberNode& n,
                        std::ostream& os, int indent) {
  os << pad(indent) << "StructMemberNode"
     << (n.is_public ? " pub" : "") << "\n";
...
void dump_impl(const Node& node, std::ostream& os, int indent) {
  ...
  std::visit(overloaded{
    ...
    [&](const KeyValueNode& n) {
      dump_key_value(n, os, indent);
    },
    ...
    [&](const FuncTypeNode& n) {
      ...
      for (const auto& p : n.params)  dump_ptr(p, os, c);
      for (const auto& r : n.returns) dump_ptr(r, os, c);
    },
    [&](const RangeTypeNode& n) {
      os << pad(indent) << "RangeTypeNode\n";
      dump_ptr(n.element_type, os, c);
    },
    [&](const FieldSpecNode& n) {
      dump_field_spec(n, os, indent);
    },
    ...
    [&](const CaseArmNode& n) {
      dump_case_arm(n, os, indent);
    },
    [&](const ParameterNode& n) {
      dump_parameter(n, os, indent);
    },
    [&](const SignatureNode& n) {
      dump_signature(n, os, indent);
    },
    ...
    [&](const ForExprNode& n) {
      ...
      if (n.accumulator) os << " acc=\"" << n.accumulator->name << "\"";
      os << "\n";
    ...
    [&](const AssignNode& n) {
      ...
      for (const auto& t : n.targets) dump_ptr(t, os, c);
      for (const auto& v : n.values)  dump_ptr(v, os, c);
    },
    ...
    [&](const ConstDeclNode& n) {
      os << pad(indent) << "ConstDeclNode"
         << (n.is_public ? " pub" : "")
         << " \"" << n.name.name << "\"\n";
      dump_opt_ptr(n.type,  os, c);
      dump_ptr(n.value, os, c);
    },
    [&](const EnumFieldNode& n) {
      dump_enum_field(n, os, indent);
    },
    [&](const EnumDeclNode& n) {
      os << pad(indent) << "EnumDeclNode"
         << (n.is_public ? " pub" : "")
         << " \"" << n.name.name << "\"\n";
      for (const auto& f : n.fields)
        dump_enum_field(f, os, c);
    },
    [&](const ReceiverNode& n) {
      dump_receiver(n, os, indent);
    },
    [&](const FuncDeclNode& n) {
      os << pad(indent) << "FuncDeclNode"
         << (n.is_public ? " pub" : "")
         << " \"" << n.name.name << "\"\n";
      dump_opt_generic(n.generic, os, c);
      if (n.receiver) dump_receiver(*n.receiver, os, c);
      dump_signature(n.signature, os, c);
      dump_ptr(n.body, os, c);
    },
    ...
    [&](const StructMemberNode& n) {
      dump_struct_member(n, os, indent);
    },
    [&](const StructDeclNode& n) {
      os << pad(indent) << "StructDeclNode"
         << (n.is_public ? " pub" : "")
         << " \"" << n.name.name << "\"\n";
      dump_opt_generic(n.generic, os, c);
