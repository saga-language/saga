...
TEST(YieldTest, ResetsReductionCount) {
  ...
  saga_runtime_set_current_actor_for_test(a);
  saga_actor_yield();
  saga_runtime_set_current_actor_for_test(nullptr);
...
static void yield_then_complete(saga_runtime_actor *a) {
  (void)a;
  saga_actor_yield();
...
TEST_F(YieldExecutorTest, YieldDoesNotPreventCompletion) {
  saga_runtime_actor *a = saga_executor_spawn(yield_then_complete, nullptr, 0, 0);
  ASSERT_NE(a, nullptr);
...
static void yield_prevents_quota_kill(saga_runtime_actor *a) {
  for (int i = 0; i < 200; i++) {
...
TEST_F(YieldExecutorTest, YieldResetsQuotaInLoop) {
  saga_runtime_actor *a = saga_executor_spawn(yield_prevents_quota_kill, nullptr, 0, 0);
  ASSERT_NE(a, nullptr);
...
/* Helper: create a heap saga_runtime_string for test use. */
static saga_runtime_string *make_test_string(const char *buf, int64_t len) {
...
static void free_test_string(saga_runtime_string *s) {
  if (s) {
    free((void *)s->data);
    free(s);
  }
...
TEST(TrapTest, SetsZombieStatusAndLongjmps) {
  ...
  saga_runtime_string *reason = make_test_string("test trap", 9);
  ...
  EXPECT_TRUE(jumped);
  EXPECT_EQ(a->status, SAGA_RUNTIME_ACTOR_ZOMBIE);
  ASSERT_NE(a->result, nullptr);

  /* Verify the reason string was copied. */
  saga_runtime_string *stored = (saga_runtime_string *)a->result;
...
static void trap_with_reason(saga_runtime_actor *a) {
  /* Allocate the reason string in the arena. */
  saga_runtime_string *reason = saga_runtime_arena_alloc_string(a->arena, "fatal error", 11);
  saga_actor_trap(reason);
  /* Should never reach here. */
}

TEST_F(TrapExecutorTest, TrapSetsZombieStatus) {
  saga_runtime_actor *a = saga_executor_spawn(trap_with_reason, nullptr, 0, 0);
  ASSERT_NE(a, nullptr);
  ...
  EXPECT_GE(status, SAGA_RUNTIME_ACTOR_COMPLETED);

  saga_task_drop(a);
}

/* An actor that traps without a reason. */
static void trap_no_reason(saga_runtime_actor *a) {
  (void)a;
  saga_actor_trap(nullptr);
}

TEST_F(TrapExecutorTest, TrapWithoutReasonCompletes) {
  saga_runtime_actor *a = saga_executor_spawn(trap_no_reason, nullptr, 0, 0);
  ASSERT_NE(a, nullptr);
...
static void trap_deep_inner(void) {
  saga_runtime_string *reason = saga_runtime_arena_alloc_string(
      saga_runtime_get_current_actor()->arena, "boom", 4);
  saga_actor_trap(reason);
}
...
static void exit_with_forty_two(saga_runtime_actor *a) {
  int64_t val = 42;
  saga_context_exit(a, &val, sizeof(val));
}

TEST_F(TrapExecutorTest, ExitResultSurvivesLongjmp) {
  saga_runtime_actor *a = saga_executor_spawn(exit_with_forty_two, nullptr, 0, 0);
  ASSERT_NE(a, nullptr);
